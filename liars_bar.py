...
class Player:
    ...
    def shoot(self) -> bool:
        ...
        if self.do_video:
            ...
            self.play_video_subprocess(self.shoots == self.fatal_shoot)
        return self.shoots == self.fatal_shoot

    def play_video_subprocess(self, is_dead: bool) -> None:
        """Call the play_video function from subprocess"""
        p = multiprocessing.Process(target=play_video, args=(is_dead,))
        p.start()
        p.join()
        return None
